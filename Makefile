...
install:
	...
	-sudo apt install -y postgresql postgresql-client cmake g++ libpq-dev libcurl4-openssl-dev libssl-dev nodejs npm 2>/dev/null
	-sudo service postgresql start
	cd frontend && npm install

db:
	@echo "=== Создание базы данных ==="
	-sudo -u postgres psql -c "CREATE USER freelancer WITH PASSWORD 'freelancer_pass';" 2>/dev/null
	-sudo -u postgres psql -c "CREATE DATABASE freelance_marketplace OWNER freelancer;" 2>/dev/null
	-sudo -u postgres psql -d freelance_marketplace -c "GRANT ALL ON SCHEMA public TO freelancer;" 2>/dev/null
